...
from va_gl import draw_circle, draw_arrow, draw_sun
import va_utils
from va_utils import get_matrix_element_square, Null
    ...
    for ob in obs:
        ...
        if ob.type == 'MESH':
            ...
            for me in meshes:
                for v in me.vertices:
                    ...
                    vec = Vector(world_to_window_coordinate( \
                                      v.co * ob.matrix_world, persmat, sx, sy))
            ...
            for bone in bones:
                ...
                for v in (head, tail):
                    vec = Vector(world_to_window_coordinate( \
                                         v * ob.matrix_world, persmat, sx, sy))
                    if 0 <= vec[0] < sx and 0 <= vec[1] < sy:
                        #row = int(vec[1] / sy * subdivide)
                        #col = int(vec[0] / sx * subdivide)
                        row = int(vec[1] / max(sx, sy) * subdivide)
