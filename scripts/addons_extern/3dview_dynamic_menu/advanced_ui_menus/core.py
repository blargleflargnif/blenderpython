...
def get_selected():
    ...
    sel_verts_num = int(sel_verts_num[0])
    ...
    sel_edges_num = int(sel_edges_num[0])
    ...
    sel_faces_num = int(sel_faces_num[0])
...
def opposingkeys(keymodes, activation, startup=False):
    
    # make it so that if you are unregistering the addon it only loops through once
    if startup:
        loops =5
    ...
    for loop in range(loops):
        ...
        for key in keymodes:
            ...
            for mode in key[0]:
                ...
                # this iterates through all the keys in the current hotkey layout and (de)activates the ones that
                # match the key we want to (de)activate
                if km:
                    for kmi in km.keymap_items:
                        if kmi.type == key[1] and kmi.any == key[2] \
                            and kmi.shift == key[3] and kmi.ctrl == key[4] \
                            and kmi.alt == key[5] and kmi.oskey == key[6]:
...
def set_prop(prop_type, path, **kwargs):
    ...
    # create the property
    #print("{0} = bpy.props.{1}({2})".format(path, prop_type, kwstring))
