...
class VIEW3D_PIE_Add_Other(Menu):
    ...
    def draw(self, context):
        ...
        pie.operator("object.add", text="Lattice", icon='OUTLINER_OB_LATTICE').type='LATTICE'
...
# Node Editor #
###############
class NodeEditorAddTexture(Menu):
	bl_idname="add.node.texture.image"
	...
	def draw(self, context):
		...
		pie.operator("node.add_node", text="Image Texture").type="ShaderNodeTexImage"
...
def register():
    ...
    ### NODE EDITOR ###
    bpy.utils.register_class(NodeEditorAddTexture)
    ...
    ###############
    #texture add
    km = wm.keyconfigs.addon.keymaps.new(name="Node Editor (Global)")
    kmi = km.keymap_items.new("wm.call_menu_pie", "A", "PRESS", shift=True)
    kmi.properties.name="add.node.texture.image"
    addon_keymaps.append(km)
...
if __name__ == "__main__":
    ...
    bpy.ops.wm.call_menu_pie(name="add.node.texture.image")
