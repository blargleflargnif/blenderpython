...
class ExternalEdit(bpy.types.Operator):
	...
	def execute(self, context):
		...
		if (self.index == 1):
			editor_path = context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_1
		elif (self.index == 2):
			editor_path = context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_2
		elif (self.index == 3):
			editor_path = context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_3
...
def menu(self, context):
	if (IsMenuEnable(__name__.split('.')[-1])):
		...
		if (context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_1):
			path = os.path.basename(context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_1)
			name, ext = os.path.splitext(path)
			self.layout.operator(ExternalEdit.bl_idname, icon="PLUGIN", text=name+" In the open").index = 1
		if (context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_2):
			path = os.path.basename(context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_2)
			name, ext = os.path.splitext(path)
			self.layout.operator(ExternalEdit.bl_idname, icon="PLUGIN", text=name+" In the open").index = 2
		if (context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_3):
			path = os.path.basename(context.user_preferences.addons["Addon Factory"].preferences.text_editor_path_3)
			name, ext = os.path.splitext(path)
