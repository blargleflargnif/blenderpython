...
class CopyBoneTransform(bpy.types.Operator):
	...
	bl_label = "Copy the bone deformation"
...
def menu(self, context):
	if (IsMenuEnable(__name__.split('.')[-1])):
		...
		op = row.operator(CopyBoneTransform.bl_idname, icon='MAN_ROT', text="Rotate copy")
		op.copy_location, op.copy_rotation, op.copy_scale = False, True, False
		op = row.operator(CopyBoneTransform.bl_idname, icon='MAN_SCALE', text="Extension of copy")
