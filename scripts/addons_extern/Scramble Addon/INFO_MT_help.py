...
class RegisterLastCommandKeyconfig(bpy.types.Operator):
	...
	items = [
		('LEFTMOUSE', "Left Mouse", "", 1),
		('MIDDLEMOUSE', "Middle Mouse", "", 2),
		('RIGHTMOUSE', "Right Mouse", "", 3),
		('BUTTON4MOUSE', "Mouse Button 4", "", 4),
		('BUTTON5MOUSE', "Mouse Button 5", "", 5),
		('BUTTON6MOUSE', "Mouse Button 6", "", 6),
		('BUTTON7MOUSE', "Mouse Button 7", "", 7),
		('MOUSEMOVE', "Mouse Move", "", 8),
		('INBETWEEN_MOUSEMOVE', "Boundry Mouse Move", "", 9),
	...
	type = bpy.props.EnumProperty(items=items, name="Input key")
	...
	def execute(self, context):
		...
		self.report(type={"INFO"}, message="You have created a shortcut, please save user settings, if necessary")
		return {'FINISHED'}
	def invoke(self, context, event):
		...
		if (commands[-1] == ''):
			commands = commands[:-1]
		if (len(commands) <= 0):
			self.report(type={'ERROR'}, message="Last command that was executed can not be found")
			return {'CANCELLED'}
		commands.reverse()
		for command in commands:
			if (re.search(r"^bpy\.ops\.wm\.call_menu", command)):
				...
				if (re.search(r"True$", command) or re.search(r"False$", command)):
					...
					self.report(type={'ERROR'}, message="The type of command that does not correspond to")
					return {'CANCELLED'}
				break
		else:
			self.report(type={'ERROR'}, message="You can register command was not found")
			return {'CANCELLED'}
		return context.window_manager.invoke_props_dialog(self)
...
class ToggleDisabledMenu(bpy.types.Operator):
	...
	bl_description = "I will switch the display / non-display of additional items of on / off button at the end of the menu byScrambleAddon"
	bl_options = {'REGISTER', 'UNDO'}
