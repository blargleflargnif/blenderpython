...
class AddSphereOnlySquare(bpy.types.Operator):
	...
	bl_description = "I'll add a sphere mesh that are configured only by a square polygon"
	bl_options = {'REGISTER', 'UNDO'}
	
	level = bpy.props.IntProperty(name="Division number", default=2, step=1, min=1, max=6, soft_min=1, soft_max=6)
	radius = bpy.props.FloatProperty(name="radius", default=1.0, step=10, precision=3, min=0.001, max=100, soft_min=0.001, soft_max=100)
	view_align = bpy.props.BoolProperty(name="View Align", default=False)
	...
	def execute(self, context):
		...
		context.active_object.name = "Polygon Sphere"
...
class AddVertexOnlyObject(bpy.types.Operator):
	...
	bl_label = "Vertex only"
	bl_description = "I'll add one vertex only of mesh object to the position of the 3D cursor"
	bl_options = {'REGISTER', 'UNDO'}
