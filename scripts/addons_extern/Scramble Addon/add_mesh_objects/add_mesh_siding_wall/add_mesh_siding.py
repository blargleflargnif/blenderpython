            ...
            if is_slope == False or cur_z + b_z < square: #flat
                ...
                if cur_z + b_z < oh: #do normal sloped boards
                    ...
                    if cur_x + bl <= cur_x + x_dif or cur_x + bl >= last_x - x_dif: #recalculate bl
                        ...
                        bl = r_float((l / 2) - v, (l / 2) + v); bl += x_dif                      
                    if is_length_vary == True:                                                               
                    ...
                    if enter == True: #single board
                        ...
                        if cur_x == start_x: #left board                         
                            if cur_x + bl > last_x - x_dif or x_dif > bl:
                                l = (last_x - x_dif) - (cur_x + x_dif); v = l * (length_vary * 0.45); bl = r_float(l - v, x_dif + v)
                            v = ((cur_x, -ohi, cur_z), (cur_x, -oi, cur_z), (cur_x + bl, -oi, cur_z), (cur_x + bl, -ohi, cur_z)) #Bottom Row
    ...
    while cur_x < ow: #main loop for width
        ...
        while cur_z < last_z: #while height it not enough
            ...
            if is_length_vary == True and counter != max_boards: #calculate board length
                v = oh* (length_vary * 0.45); bl = r_float((oh/ 2) - v, (oh / 2) + v)
            else:
                bl = oh
            if cur_z + bl > oh:
                bl = oh - cur_z            
...
def vinyl_lap(oh, ow, is_slope, slope, is_length_vary, length_vary, bw, faces, verts, max_boards): #vinyl horizontal lapped siding
    ...
    while cur_z < oh:
        ...
        while cur_x < last_x:
            ...
            if is_length_vary == True:
                l = (last_x - x_dif) - (start_x + x_dif); v = l * length_vary * 0.49; bl = r_float((l / 2) - v, (l / 2) + v); bl += x_dif
...
def bricks_mortar(oh, ow, m_d): #creates mortar object
    verts = []; faces = []; depth = 3.5 / 39.3701; y = depth - m_d
...
def addSiding(context, mat, if_tin, if_wood, if_vinyl, is_slope, over_width, over_height, board_width,
    ...
    if mat == "1" and if_wood == "1": #Wood > Vertical
        ...
        for i in batten_pos[1]:
            ...
            if cur_x + baw < ow:
                p = len(verts)
                verts.append((cur_x, -0.02539, 0.0)); verts.append((cur_x, -0.04444, 0.0)) #Bottom > Left
                verts.append((cur_x + baw, -0.04444, 0.0)); verts.append((cur_x + baw, -0.02539, 0.0)) #Bottom > Right
                verts.append((cur_x, -0.02539, cur_z)); verts.append((cur_x, -0.04444, cur_z)) #Top > Left
                if is_slope == False: #flat
                    verts.append((cur_x + baw, -0.04444, oh)); verts.append((cur_x + baw, -0.02539, oh)) #Top > Right 
                elif is_slope == True:    
                    if cur_x < ow /2 and cur_x + baw < ow / 2: #slope up
                        verts.append((cur_x + baw, -0.04444, cur_z + z_dif)); verts.append((cur_x + baw, -0.02539, cur_z + z_dif)) #Top > Right
                    elif cur_x < ow / 2 and cur_x + baw > ow / 2: #middle board
                        del verts[len(verts) - 1]; del verts[len(verts) - 1] #remove top > right set         
                        verts.insert(len(verts) - 2, (ow / 2, -0.04444, 0.0)); verts.insert(len(verts) - 2, (ow / 2, -0.02539, 0.0)) #insert bottom middle set
                        z_pos = oh - ((slope * ((ow / 2) - cur_x)) / 12) #figure out whats on left and calculate height
                        verts.append((cur_x, -0.02539, z_pos)); verts.append((cur_x, -0.04444, z_pos)) #Top > Left
                        verts.append((ow / 2, -0.04444, oh)); verts.append((ow / 2, -0.02539, oh)) #Top > Middle
                        z_pos = oh - ((slope * ((cur_x + baw) - (ow / 2))) / 12) #figure out what on right and calculate height
                        verts.append((cur_x + baw, -0.04444, z_pos)); verts.append((cur_x + baw, -0.02539, z_pos)); is_center = True #Top > Right
                    elif cur_x > ow / 2: #slope down
                        verts.append((cur_x + baw, -0.04444, cur_z - z_dif)); verts.append((cur_x + baw, -0.02539, cur_z - z_dif)) #Top > Right
                if is_center == False:
...
class MEST_OT_primitive_add_siding(bpy.types.Operator):
    ...
    def draw(self, context):
        layout = self.layout      
        layout.label("Material:"); layout.prop(self, "mat", icon = "MATERIAL")
        ...
        if self.object_cut == False:
            layout.prop(self, "over_width"); layout.prop(self, "over_height"); layout.separator()
        elif self.object_cut == True:
            layout.label("Using Active Object", icon = "FACESEL")
            layout.prop(self, "x_off"); layout.separator()
        ...
        if self.mat != "5":
            ...
            layout.prop(self, "b_width"); layout.prop(self, "b_height")
            layout.prop(self, "b_ran_offset", icon = "NLA")
            if self.b_ran_offset == False: layout.prop(self, "b_offset")
            else: layout.prop(self, "b_vary") 
            layout.separator(); layout.prop(self, "b_gap")
            layout.prop(self, "m_depth"); layout.prop(self, "is_bevel", icon = "MOD_BEVEL")
        ...
        if self.object_cut == False:
            ...
            if self.is_cutout == True:
                ...
                for i in range(1, self.num_cutouts + 1):
                    layout.label("Cutout " + str(i) + ":", icon = "MOD_BOOLEAN"); layout.prop(self, "nc" + str(i))
        layout.separator(); layout.prop(self, "is_material", icon = "MATERIAL"); layout.separator()
        if self.is_material == True:
            if self.mat in ("2", "3"): #tin and vinyl
                ...
                layout.prop(self, "color_style", icon = "COLOR"); layout.prop(self, "mat_color", icon = "COLOR")
                if self.color_style != "constant": layout.prop(self, "mat_color2", icon = "COLOR")
                if self.color_style == "extreme": layout.prop(self, "mat_color3", icon = "COLOR")
                layout.prop(self, "color_sharp"); layout.prop(self, "color_scale"); layout.separator(); layout.prop(self, "mortar_color", icon = "COLOR")
                layout.prop(self, "mortar_bump"); layout.prop(self, "bump_type", icon = "SMOOTHCURVE")
                if self.bump_type != "4": layout.prop(self, "brick_bump"); layout.prop(self, "bump_scale")                      
            layout.separator(); layout.prop(self, "is_preview", icon = "SCENE")
        layout.label("Rotation (XYZ):", icon = "MAN_ROT"); layout.prop(self, "rotation")
        layout.label("Location:", icon = "MAN_TRANS"); layout.prop(self, "location")
    
    def execute(self, context):       
        if bpy.context.scene.render.engine != "CYCLES": #make cycles
            bpy.context.scene.render.engine == "CYCLES"
        ob = bpy.context.scene.objects.active; enter = False
        if ob != None:
            if ob.type == "MESH":
        ...
        if self.object_cut == True and enter == True:                        
            pattern = bpy.context.scene.objects.active; pattern_dim = tuple(pattern.dimensions)
            dims = []; dims2 = []
            for i in pattern_dim: i += 0.1; i *= 3.28084; dims.append(i)
            for i in pattern_dim: i += 0.1; dims2.append(i)
        ...
        if self.object_cut == False or enter == False:
            ...
            eur = (pattern.rotation_euler).copy(); eur.rotate_axis("X", radians(-90.0)) #subtract rotation from object to make up for rotating up
            ob.rotation_euler = eur
        #add mortar if bricks and materials
        if self.mat == "5":
            try: data_back = bricks_mortar(dims2[1], dims2[0], round(self.m_depth / 39.3701, 5))
            except: data_back = bricks_mortar(round(self.over_height / 3.28084, 5), round(self.over_width / 3.28084, 5), round(self.m_depth / 39.3701, 5))
            m_verts = data_back[0]; m_faces = data_back[1] #create mortar object
            ...
            mortar.location = ob.location; mortar.rotation_euler = ob.rotation_euler #set position and rotation
            if self.is_material == True:
                try: m_mat = bpy.data.materials.get("mortar"); m_mat.use_nodes = True #add materials
                except: m_mat = bpy.data.materials.new("mortar"); m_mat.use_nodes = True
                #figure brick name
                b_name = "brick_" + str(self.color_style) + "_" + str(self.bump_type)
                try: b_mat = bpy.data.materials.get(b_name); b_mat.use_nodes = True
                except: b_mat = bpy.data.materials.new(b_name); b_mat.use_nodes = True            
                ob.data.materials.append(b_mat); mortar.data.materials.append(m_mat)
            if self.is_slope == False and (self.object_cut == False or enter == False) and self.is_cutout == False:
                already_selected = []; scene = context.scene #join mortar and bricks object
                for obs in scene.objects:
                    if obs.select == True: already_selected.append(obs.name)
                    obs.select = False                    
                mortar.select = True; ob.select = True; bpy.context.scene.objects.active = ob 
                name = ob.name; bpy.ops.object.join(); ob = bpy.data.objects[name]
        ...
        if self.mat == "5" and self.is_slope == True:
            ...
            s_verts = data_back[0]; s_faces = data_back[1]
            ...
            cut.location = ob.location; cut.rotation_euler = ob.rotation_euler
            ...
            bpy.context.object.modifiers["Boolean"].operation = "DIFFERENCE"
            bpy.ops.object.modifier_apply(apply_as = "DATA", modifier = "Boolean")
            already_selected = []; scene = context.scene
            for obs in scene.objects:
            ...
            if (self.object_cut == False or enter == False) and self.is_cutout == False:
                mortar.select = True; ob.select = True; bpy.context.scene.objects.active = ob; bpy.ops.object.join()
            for i in already_selected:
                (bpy.data.objects[i]).select = True
            ...
            if self.if_vinyl == "3": bpy.context.object.modifiers["Bevel"].use_clamp_overlap = False
        ...
        if self.is_cutout == True:
            ...
            if bool_stuff[0] != []:
                ...
                if self.mat == "5": #bricks
                    bpy.context.scene.objects.active = mortar; bpy.ops.object.modifier_add(type = "BOOLEAN")
                    bpy.context.object.modifiers["Boolean"].object = bpy.data.objects[bool_ob.name]                  
                    bpy.context.object.modifiers["Boolean"].operation = "DIFFERENCE"    
                    bpy.ops.object.modifier_apply(apply_as = "DATA", modifier = "Boolean")
                scene = context.scene
        ...
        #materials
        bpy.context.scene.objects.active = ob
        rgba = list(self.mat_color); rgba.append(1.0)       
        if self.is_material == True and self.mat not in ("1", "5"): #tin or vinyl or fibercement
            if self.mat == "2": mat_name = "vinyl_siding"; rough = 0.3
            elif self.mat == "3": mat_name = "tin_siding"; rough = 0.18
            elif self.mat == "4": mat_name = "fibercement_siding"; rough = 0.35 
            try:
                ...
                mat.use_nodes = True
                nodes = mat.node_tree.nodes
                node = nodes["Diffuse BSDF"]; mat.node_tree.nodes.remove(node)
                node = nodes.new("ShaderNodeBsdfDiffuse")
                node.inputs[0].default_value = rgba; node.location = -100, 400
                mat.diffuse_color = (rgba[0], rgba[1], rgba[2])
                node = nodes.new("ShaderNodeBsdfGlossy"); node.name = "Glossy"
                node.inputs[1].default_value = rough
                node.location = -100, 200
                node = nodes.new("ShaderNodeMixShader"); node.name = "Mix"
                node.location = 100, 300; node.inputs[0].default_value = 0.05
                outN = nodes["Diffuse BSDF"].outputs[0]; inN = nodes["Mix"].inputs[1]
                mat.node_tree.links.new(outN, inN)
                outN = nodes["Glossy"].outputs[0]; inN = nodes["Mix"].inputs[2]
                mat.node_tree.links.new(outN, inN)
                outN = nodes["Mix"].outputs[0]; inN = nodes["Material Output"].inputs[0]
                mat.node_tree.links.new(outN, inN)
                ob.data.materials.append(mat)
        elif self.is_material == True and self.mat == "1": #wood
            ob.select = True; mat_name = "wood_siding"            
            ...
            mat_enter = False; mat_enter2 = False            
            for im in bpy.data.images: #check for images
                if im.filepath == self.col_image and mat_enter == False:
                    col_im = im; enter = True
                if im.filepath == self.norm_image and mat_enter2 == False and self.is_bump == True:
                    norm_im = im; mat_enter2 = True
            if mat_enter == False and self.col_image != "": col_im = bpy.data.images.load(self.col_image)            
            if self.is_bump == True and mat_enter2 == False and self.norm_image != "": norm_im = bpy.data.images.load(self.norm_image)
            try: #update current material
                mat = bpy.data.materials.get(mat_name)
                mat.use_nodes = True; nodes = mat.node_tree.nodes; node = nodes["scale"]; node.scale = (self.im_scale, self.im_scale, self.im_scale)
                node = nodes["color_image"]; node.image = col_im; node = nodes["normal_image"]
                if self.is_bump == True: node.image = norm_im
                node = nodes["normal_map"]; node.inputs[0].default_value = self.bump_amo; node = nodes["math"]
                if self.is_bump == True: node.inputs[1].default_value = 1.0
                else: node.inputs[1].default_value = 0.0
                ob.data.materials.append(mat)
            except:
                mat = bpy.data.materials.new(mat_name)
                mat.use_nodes = True
                ...
                node.location = -700, 0; node.name = "uv_coords"
                node = nodes.new("ShaderNodeMapping")
                node.location = -500, 0; node.name = "scale"; node.scale = (self.im_scale, self.im_scale, self.im_scale)
                node = nodes.new("ShaderNodeTexImage")
                node.location = -100, 150; node.name = "color_image"; node.image = col_im
                node = nodes.new("ShaderNodeTexImage")
                node.location = -100, -150; node.name = "normal_image"
                if self.is_bump == True: node.image = norm_im
                node = nodes["Diffuse BSDF"]
                node.location = 100, 100; node.name = "diffuse"
                node = nodes.new("ShaderNodeNormalMap")
                node.location = 100, -100; node.name = "normal_map"; node.inputs[0].default_value = self.bump_amo
                node = nodes.new("ShaderNodeMath")
                node.location = 300, -100; node.name = "math"; node.operation = "MULTIPLY"; 
                if self.is_bump == True: node.inputs[1].default_value = 1.0
                else: node.inputs[1].default_value = 0.0
                node = nodes["Material Output"]; node.location = 500, 0
                o = nodes["uv_coords"].outputs[2]; i = nodes["scale"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["scale"].outputs[0]; i = nodes["color_image"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["scale"].outputs[0]; i = nodes["normal_image"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["color_image"].outputs[0]; i = nodes["diffuse"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["normal_image"].outputs[0]; i = nodes["normal_map"].inputs[1]; mat.node_tree.links.new(o, i)
                o = nodes["normal_map"].outputs[0]; i = nodes["math"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["math"].outputs[0]; i = nodes["Material Output"].inputs[2]; mat.node_tree.links.new(o, i)
                ob.data.materials.append(mat)
                ...
                if area.type == 'VIEW_3D':
                    for region in area.regions:
                        if region.type == 'WINDOW':
                            ...
                            override = bpy.context.copy(); override["area"] = area; override["region"] = region; override["active_object"] = (bpy.context.selected_objects)[0]; bpy.ops.mesh.select_all()
                            bpy.ops.uv.cube_project(override); bpy.ops.object.editmode_toggle()
            #mortar
            mat = bpy.data.materials.get("mortar"); mat.use_nodes = True; nodes = mat.node_tree.nodes; mat.diffuse_color = self.mortar_color
            if len(nodes) > 2: #update
                node = nodes["Diffuse BSDF"]; node.inputs[0].default_value = (self.mortar_color[0], self.mortar_color[1], self.mortar_color[2], 1.0)
                node = nodes["b_amount"]; node.inputs[1].default_value = self.mortar_bump
            else: #create
                node = nodes["Diffuse BSDF"]; node.inputs[0].default_value = (self.mortar_color[0], self.mortar_color[1], self.mortar_color[2], 1.0)
                node = nodes.new("ShaderNodeTexCoord"); node.name = "uv_coords"; node.location = -700, 200
                node = nodes.new("ShaderNodeMapping"); node.name = "scale"; node.location = -500, 200; node.scale = (15.0, 15.0, 15.0)
                node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise"; node.location = -150, 200; node.inputs[1].default_value = 100.0
                node = nodes.new("ShaderNodeMath"); node.name = "b_amount"; node.location = 50, 100; node.operation = "MULTIPLY"; node.inputs[1].default_value = self.mortar_bump
                #connections
                o = nodes["uv_coords"].outputs[2]; i = nodes["scale"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["scale"].outputs[0]; i = nodes["b_noise"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["b_noise"].outputs[1]; i = nodes["b_amount"].inputs[0]; mat.node_tree.links.new(o, i)
                o = nodes["b_amount"].outputs[0]; i = nodes["Material Output"].inputs[2]; mat.node_tree.links.new(o, i)
            #brick
            b_name = "brick_" + str(self.color_style) + "_" + str(self.bump_type)
            mat = bpy.data.materials.get(b_name); mat.use_nodes = True
            nodes = mat.node_tree.nodes; mat.diffuse_color = self.mat_color
            if self.color_style == "constant": #single color
                node = nodes["Diffuse BSDF"]; node.inputs[0].default_value = (self.mat_color[0], self.mat_color[1], self.mat_color[2], 1.0)
            elif self.color_style == "speckled": #speckled
                if len(nodes) > 2: #update
                    node = nodes["scale"]; node.scale = (self.color_scale * 2.0, self.color_scale * 2.0, self.color_scale * 2.0)
                    node = nodes["color_vary"]; node.inputs[2].default_value = self.color_sharp * 2
                    nodes["color1"].outputs[0].default_value = (self.mat_color[0], self.mat_color[1], self.mat_color[2], 1.0); nodes["color2"].outputs[0].default_value = (self.mat_color2[0], self.mat_color2[1], self.mat_color2[2], 1.0)
                else: #create
                    node = nodes.new("ShaderNodeTexCoord"); node.name = "uv_coords"; node.location = -1500, 200
                    node = nodes.new("ShaderNodeMapping"); node.name = "scale"; node.location = -1300, 200; node.scale = (self.color_scale * 2.0, self.color_scale * 2.0, self.color_scale * 2.0); node.label = "Color Scale"
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise1"; node.location = -1200, 400; node.inputs[1].default_value = 2.0
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise2"; node.location = -1200, 600
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "cn1|cn2"; node.location = -1000, 475
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "color_vary"; node.location = -850, 400; node.inputs[2].default_value = self.color_sharp * 2.0; node.label = "Color Sharpness"
                    node = nodes.new("ShaderNodeRGB"); node.name = "color1"; node.location = -900, 50; node.outputs[0].default_value = (self.mat_color[0], self.mat_color[1], self.mat_color[2], 1.0); node.label = "Color 1"
                    node = nodes.new("ShaderNodeRGB"); node.name = "color2"; node.location = -900, 250; node.outputs[0].default_value = (self.mat_color2[0], self.mat_color2[1], self.mat_color2[2], 1.0); node.label = "Color 2"
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "c1|c2"; node.location = -700, 300
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "c1Xc2"; node.location = -700, 100; node.blend_type = "MULTIPLY"
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise3"; node.location = -1000, 700; node.inputs[1].default_value = 600.0
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "contrast"; node.location = -800, 700; node.inputs[2].default_value = 6.0
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise4"; node.location = -800, 575; node.inputs[1].default_value = 1000.0
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "con|cn4"; node.location = -600, 600; node.inputs[1].default_value = (0.0, 0.0, 0.0, 1.0)
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "contrast2"; node.location = -400, 400; node.inputs[2].default_value = 4.0
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "final|"; node.location = -200, 300
                    ...
                    for i in links:
                        o = nodes[i[0]].outputs[i[1]]; i = nodes[i[2]].inputs[i[3]]; mat.node_tree.links.new(o, i)
            elif self.color_style == "multiple": #multiple mixed
                if len(nodes) > 2: #update
                    nodes["scale"].scale = (self.color_scale, self.color_scale, self.color_scale); nodes["color_vary"].inputs[2].default_value = self.color_sharp
                    ...
                    node = nodes.new("ShaderNodeTexCoord"); node.name = "uv_coords"; node.location = -1100, 300
                    node = nodes.new("ShaderNodeMapping"); node.name = "scale"; node.location = -925, 350; node.scale = (self.color_scale, self.color_scale, self.color_scale); node.label = "Color Scale"
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise"; node.location = -600, 600; node.inputs[1].default_value = 4.0
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "color_vary"; node.location = -400, 500; node.inputs[2].default_value = self.color_sharp; node.label = "Color Sharpness"
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "c1|c2"; node.location = -200, 450
                    node = nodes.new("ShaderNodeRGB"); node.name = "color1"; node.location = -500, 175; node.outputs[0].default_value = (self.mat_color[0], self.mat_color[1], self.mat_color[2], 1.0); node.label = "Color 1"
                    node = nodes.new("ShaderNodeRGB"); node.name = "color2"; node.location = -500, 375; node.outputs[0].default_value = (self.mat_color2[0], self.mat_color2[1], self.mat_color2[2], 1.0); node.label = "Color 2"
                ...
                if len(nodes) > 2: #update
                    nodes["scale"].scale = (self.color_scale, self.color_scale, self.color_scale); nodes["c_v1"].inputs[2].default_value = self.color_sharp
                    nodes["color1"].outputs[0].default_value = (self.mat_color[0], self.mat_color[1], self.mat_color[2], 1.0)
                    nodes["color2"].outputs[0].default_value = (self.mat_color2[0], self.mat_color2[1], self.mat_color2[2], 1.0)
                    nodes["color3"].outputs[0].default_value = (self.mat_color3[0], self.mat_color3[1], self.mat_color3[2], 1.0)
                    nodes["c_v2"].inputs[2].default_value = self.color_sharp * 1.25; nodes["c_v3"].inputs[2].default_value = self.color_sharp * 1.25 
                else: #create
                    node = nodes.new("ShaderNodeTexCoord"); node.name = "uv_coords"; node.location = -1700, 400
                    node = nodes.new("ShaderNodeMapping"); node.name = "scale"; node.location = -1500, 400; node.scale = (self.color_scale, self.color_scale, self.color_scale); node.label = "Color Scale"
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise"; node.location = -1100, 600; node.inputs[1].default_value = 10.0
                    node = nodes.new("ShaderNodeRGB"); node.name = "color3"; node.location = -1150, 800; node.outputs[0].default_value = (self.mat_color3[0], self.mat_color3[1], self.mat_color3[2], 1.0); node.label = "Color 3"
                    node = nodes.new("ShaderNodeRGB"); node.name = "color2"; node.location = -1050, 400; node.outputs[0].default_value = (self.mat_color2[0], self.mat_color2[1], self.mat_color2[2], 1.0); node.label = "Color 2"                    
                    node = nodes.new("ShaderNodeRGB"); node.name = "color1"; node.location = -1050, 200; node.outputs[0].default_value = (self.mat_color[0], self.mat_color[1], self.mat_color[2], 1.0); node.label = "Color 1"
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "c_v1"; node.location = -900, 525; node.inputs[2].default_value = self.color_sharp; node.label = "Color Sharpness 1"
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "c1|c2"; node.location = -700, 400
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise2"; node.location = -900, 800
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "c_v2"; node.location = -700, 700; node.inputs[2].default_value = self.color_sharp * 1.25; node.label = "Color Sharpness 2"
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "c3|c2c1"; node.location = -500, 600
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "c_noise3"; node.location = -600, 900; node.inputs[1].default_value = 50.0
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "c_v3"; node.location = -400, 800; node.inputs[2].default_value = self.color_sharp * 1.25; node.label = "Color Sharpness 3"
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "c1|c2.2"; node.location = -400, 400
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "final|"; node.location = -250, 600
            ...
            if self.bump_type == "1": #dimpled
                try: #update
                    nodes["b_scale"].scale = (self.bump_scale, self.bump_scale, self.bump_scale); nodes["b_amount"].inputs[1].default_vale = self.brick_bump
                except: #create
                    node = nodes.new("ShaderNodeMapping"); node.name = "b_scale"; node.location = -700, -100; node.scale = (self.bump_scale, self.bump_scale, self.bump_scale); node.label = "Scale Amount"; node.label = "Bump Scale"
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise"; node.location = -300, 0; node.inputs[1].default_value = 400.0
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise2"; node.location = -300, -200; node.inputs[1].default_value = 200.0
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "bn|bn2"; node.location = -100, -100
                    node = nodes.new("ShaderNodeMath"); node.name = "b_amount"; node.location = 100, 0; node.operation = "MULTIPLY"; node.inputs[1].default_value = self.brick_bump; node.label = "Bump Amount"; node.label = "Bump Amount"
                ...
                try: #update
                     nodes["b_scale"].scale = (self.bump_scale, self.bump_scale, self.bump_scale); nodes["b_amount"].inputs[1].default_value = self.brick_bump
                except: #create
                    nodes =  mat.node_tree.nodes
                    node = nodes.new("ShaderNodeMapping"); node.name = "b_scale"; node.location = -800, -100; node.rotation = (radians(-45.0), 0.0, 0.0); node.scale = (self.bump_scale, self.bump_scale, self.bump_scale); node.label = "Bump Scale"
                    node = nodes.new("ShaderNodeTexWave"); node.name = "b_wave"; node.location = -400, 0; node.inputs[1].default_value = 100.0
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise"; node.location = -400, -200; node.inputs[1].default_value = 400.0; node.inputs[3].default_value = 1.0
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "bw|bn"; node.location = -200, -100; node.blend_type = "ADD"; node.inputs[0].default_value = 1.0
                    node = nodes.new("ShaderNodeMath"); node.name = "b_amount"; node.location = 100, 0; node.operation = "MULTIPLY"; node.inputs[1].default_value = self.brick_bump; node.label = "Bump Amount"
                ...
                try: #update
                    nodes["b_scale"].scale = (self.bump_scale, self.bump_scale, self.bump_scale); nodes["b_scale2"].scale = (self.bump_scale, self.bump_scale, self.bump_scale)
                    nodes["b_amount"].inputs[1].default_value = self.brick_bump
                except:
                    node = nodes.new("ShaderNodeMapping"); node.name = "b_scale"; node.location = -1500, -700; node.scale = (self.bump_scale, self.bump_scale, self.bump_scale); node.label = "Bump Scale"
                    node = nodes.new("ShaderNodeMapping"); node.name = "b_scale2"; node.location = -1500, -400; node.rotation = (radians(-45.0), 0.0, 0.0); node.scale = (self.bump_scale, self.bump_scale, self.bump_scale); node.label = "Bump Scale 2"
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise"; node.location = -1400, -200; node.inputs[1].default_value = 40.0
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "bc"; node.location = -1200, -250; node.inputs[2].default_value = 15.0
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise2"; node.location = -1200, -50; node.inputs[1].default_value = 12.0; node.inputs[3].default_value = 1.0
                    node = nodes.new("ShaderNodeTexWave"); node.name = "wave"; node.location = -1150, -500; node.inputs[1].default_value = 30.0; node.inputs[2].default_value = 4.0
                    node = nodes.new("ShaderNodeBrightContrast"); node.name = "bc2"; node.location = -1000, -200; node.inputs[2].default_value = 15.0
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "w|bc"; node.location = -950, -425; node.inputs[1].default_value = (0.2, 0.2, 0.2, 1.0)
                    node = nodes.new("ShaderNodeTexMusgrave"); node.name = "musgrave"; node.location = -900, -600; node.inputs[1].default_value = 300.0
                    node = nodes.new("ShaderNodeTexNoise"); node.name = "b_noise3"; node.location = -800, -900; node.inputs[1].default_value = 1500.0; node.inputs[3].default_value = 0.25
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "bc2|m1"; node.location = -750, -300; node.inputs[1].default_value = (0.2, 0.2, 0.2, 1.0)
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "m2|mus"; node.location = -700, -500
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "m2|m3"; node.location = -500, -400; node.inputs[1].default_value = (0.2, 0.2, 0.2, 1.0)
                    node = nodes.new("ShaderNodeMath"); node.name = "math"; node.location = -500, -800; node.operation = "MULTIPLY"; node.inputs[1].default_value = 0.1
                    node = nodes.new("ShaderNodeInvert"); node.name = "invert"; node.location = -350, -600
                    node = nodes.new("ShaderNodeMixRGB"); node.name = "m3|math"; node.location = -150, -550
                    node = nodes.new("ShaderNodeMath"); node.name = "b_amount"; node.location = 100, -200; node.operation = "MULTIPLY"; node.inputs[1].default_value = self.brick_bump; node.label = "Bump Amount"
            ...
            if area.type == 'VIEW_3D':
                for space in area.spaces:
                    if space.type == 'VIEW_3D':
                        if self.is_preview == True: space.viewport_shade = 'RENDERED'
                        else: space.viewport_shade = "SOLID"
        #update rotation and location
        ob.location = ob.location + self.location
        eur = (ob.rotation_euler).copy(); tup = tuple(self.rotation); eur.rotate_axis("X", tup[0]); eur.rotate_axis("Y", tup[1]); eur.rotate_axis("Z", tup[2]) 
        ob.rotation_euler = eur
        return {"FINISHED"}
