...
class CopyOtherUVMenuOperator(bpy.types.Operator): #
	...
	bl_description = "'m going to copy the active UV deployment from other UV"
	...
	def execute(self, context):
		...
		if (len(obj.data.uv_layers) < 2):
			self.report(type={"ERROR"}, message="Run in the UV number of two or more")
			return {"CANCELLED"}
...
class CopyOtherUVMenu(bpy.types.Menu):
	...
	bl_label = "Copied from other UV"
	bl_description = "I'm going to copy the active UV deployment from other UV"
...
class CopyOtherUV(bpy.types.Operator):
	...
	bl_label = "Copied from other UV"
	bl_description = "Active UV deployment of selected portions, I'm going to copy from other UV"
	bl_options = {'REGISTER', 'UNDO'}
	
	uv = bpy.props.StringProperty(name="Copy the original UV")
