...
def draw_options(layout, context):    
    ...
    row = layout.row()
    row.prop(context.window_manager, 'mst_sort_from')
    ...
    row = layout.row()
    row.prop(context.window_manager, 'mst_connected_first')

    row = layout.row()
    row.prop(context.window_manager, 'mst_from_faces')
    
    row = layout.row()
    row.prop(context.window_manager, 'mst_reverse')


def draw_panel(self, context):
    
    if not operator_poll(context):
        return
    
    layout = self.layout
    
    layout.label("MST Sorting:")
    
    layout.operator("object.mst_sort_mesh_elements", text="MST Sort Mesh Elements")


def register(is_submodule=False):
    # Properties
    bpy.types.WindowManager.mst_sort_from = bpy.props.EnumProperty(name="Sort From", default='SELECTED', items=[('CURSOR','Cursor Distance','CURSOR'), ('RANDOMIZE','Randomize','RANDOMIZE'), ('SELECTED', 'Selected', 'SELECTED'), ('OBJECT', "Object", 'OBJECT'),('WEIGHTS', 'Weights', 'WEIGHTS')])
    bpy.types.WindowManager.mst_sort_object = bpy.props.EnumProperty(name='Object', items=sort_objects_enum)
    bpy.types.WindowManager.mst_sort_verts = bpy.props.BoolProperty(name="Sort Verts", default=True)
    bpy.types.WindowManager.mst_sort_faces = bpy.props.BoolProperty(name="Sort Faces", default=True  )
    bpy.types.WindowManager.mst_connected_first = bpy.props.BoolProperty(name="Connected Verts First", default=True)
    bpy.types.WindowManager.mst_from_faces = bpy.props.BoolProperty(name="Sort From Selected Faces", default=False)
    bpy.types.WindowManager.mst_reverse = bpy.props.BoolProperty(name="Reverse", default=False)
    ...
    # Panel
    bpy.types.VIEW3D_PT_tools_meshedit.append(draw_panel)
...
def unregister(is_submodule=False):
    ...
    # Properties
    del bpy.types.WindowManager.mst_sort_from
    del bpy.types.WindowManager.mst_sort_object
    del bpy.types.WindowManager.mst_sort_verts
    del bpy.types.WindowManager.mst_sort_faces
    del bpy.types.WindowManager.mst_connected_first
    del bpy.types.WindowManager.mst_from_faces
    del bpy.types.WindowManager.mst_reverse
