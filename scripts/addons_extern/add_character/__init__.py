...
def import_object_petunia(obname):
	...
	for p in bpy.utils.script_paths():
		...
		testfname= p + '%saddons_extern%sadd_character%spetunia_original.blend' % (s,s,s)
		print(testfname)
		if os.path.isfile(testfname):
			fname=testfname
			dpath = p + \
			'%saddons_extern%sadd_character%spetunia_original.blend\\Group\\' % (s, s, s)
			break
	# DEBUG
...
def import_object_mancandy(obname):
	...
	for p in bpy.utils.script_paths():
		...
		testfname= p + '%saddons_extern%sadd_character%smancandy.blend' % (s,s,s)
		print(testfname)
		if os.path.isfile(testfname):
			fname=testfname
			dpath = p + \
			'%saddons_extern%sadd_character%smancandy.blend\\Group\\' % (s, s, s)
			break
	# DEBUG
	#print('import_object: ' + opath)
...
class Import_Mancandy(bpy.types.Operator):
    '''Imports a rigidbody recorder'''
    bl_idname = "object.import_mancandy"
    bl_label = "Add Mancandy!"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        import_object_mancandy("Mancandy")
