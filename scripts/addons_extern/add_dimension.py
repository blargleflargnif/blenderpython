...
# Original by Alessandro Sala (Feb, 12th 2012)
def convertDistance(val, units_info, precision):
...
def createCurve(vertArray, self, align_matrix):
    ...
    stext = abs(round(self.Dimension_width, gettextround))
    if gettextround == 0:
        stext = abs(int(stext))
    unit_system = bpy.context.scene.unit_settings.system
    if unit_system == 'METRIC' or unit_system == 'IMPERIAL':
        uinfo = getUnitsInfo()
        stext = convertDistance(stext, uinfo, gettextround)
