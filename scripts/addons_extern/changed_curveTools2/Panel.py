...
class Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Z.2 batSelection
        row = boxSelection.row(align = True)
        # row.prop(context.scene.curvetools, "batNrSelectedObjects", text = "")
        # row.prop(bpy.context, "batNrSelectedObjectsScene", text = "")
        row.label("%d curves selected" % (context.scene.batNrSelectedObjectsScene))
        ...
        revolveColumn = boxSurface.column(align = True)
        # row = boxSurface.row(align = True)
        revolveColumn.operator("curvetools2.operatorrevolvecurves", text = "Revolve curves")
        # row = boxSurface.row(align = True)
        revolveColumn.prop(context.scene.curvetools, "AngularResolution", text = "AngRes")
