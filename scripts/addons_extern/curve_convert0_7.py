...
def CurveConvert(self, context):
    o = context.object; na = []; jna = []  
    if "," in o.names:
        na = o.names.split(","); del na[len(na) - 1]
    else:
        na.append(o.names)
    ...
    for i in na:
        if i in bpy.data.objects:        
            ...
            if curve.type == "CURVE":
                o.select = False; curve.select = True; context.scene.objects.active = curve
                ...
                if len(na) == 1:
                    ...
                    o.select = False; temp.select = True; context.scene.objects.active = temp
                    bpy.ops.object.delete(); o.select = True; context.scene.objects.active = o
                    if o.rscale == True or len(na) >= 2: o.scale = curve.scale 
                #other objects
                else:
    ...
    for i in jna:
        ...
        if i != jna[0]:
            bpy.data.objects[i].select = True; temp.select = True; context.scene.objects.active = temp
            bpy.ops.object.join()
    if jna != []:
        o.data = temp.data.copy(); temp.select = True; context.scene.objects.active = temp; bpy.ops.object.delete()
...
class CruveConversionAddMultiple(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for objs in nas:
            ...
            loc = list(ob.location); loc[2] += 0.5            
            bpy.ops.mesh.primitive_cube_add(location = loc)
...
class CurveConversionUpdateAll(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for n in na:
            ...
            temp.select = True; context.scene.objects.active = temp
            CurveConvert(self, context)
            temp.select = False
...
class CurveConversionPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if context.mode not in ("EDIT_MESH", "EDIT_CURVE"):
            if o != None:
                if o.type == "MESH":
                    layout.label("Curve Name:", icon = "OUTLINER_OB_CURVE")
                    if "," in o.names:
                        li = o.names.split(","); del li[len(li) - 1]
                        for i in li:
                            layout.label(i)
                    else:
                        layout.prop_search(o, "names",  context.scene, "objects"); layout.prop(o, "rscale", icon = "MAN_SCALE")
            ...
            layout.label("Exit Edit Mode To Use Curve Conversion", icon = "ERROR")      
