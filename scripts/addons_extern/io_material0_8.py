...
def export_node_type(n):
    t = n.type; is_group = False; images = []
    #basic info
    ...
    #Group
        ...
        pos = 0; temp = []; is_group = True
        for i2 in n.inputs:
            if i2.type in ("RGBA", "VECTOR"):
                temp.append(pos); temp.append(make_tuple(i2.default_value))
            elif i2.type != ("SHADER"):
                temp.append(pos); temp.append(i2.default_value)
            else:
                temp.append(pos); temp.append("SHADER")
            pos += 1
        if temp != []: i = temp              
    ...
    #layout                   
    out["node_specific"] = ns; out["inputs"] = i; out["outputs"] = o   
    return out, is_group, images

def link_info(link):
    out = [c_name(link.from_node.name)]
    fr = link.from_socket.path_from_id(); fr = fr.split(".")
    if len(fr) == 3:
        fr = fr[2]
    else:
        fr = fr[1]
    n1 = fr.index("["); n2 = fr.index("]")
    ind = int(fr[n1 + 1:n2])
    out.append(ind)
    out.append(c_name(link.to_node.name))
    fr = link.to_socket.path_from_id(); fr = fr.split(".")
    if len(fr) == 3:
        fr = fr[2]
    else:
        fr = fr[1]
...
def export_material(self, context):
    mat_list = []; et = context.scene.export_materials_type
    ...
    for mat_name in mat_list:
        mat = bpy.data.materials[mat_name]; epath = context.scene.save_path_export
        if mat != None:
            if epath != "":
                #try open file
                error = True
                ...
                if error == False:
                            ...
                            for n_group in n_main.node_tree.nodes: #nodes
                                ...
                                if is_group1 == True: #group level 2
                                    ...
                                    for n_group2 in n_group.node_tree.nodes: #nodes
                                        ...
                                        g_n2.append(g_out2); images.append(im2)
                                        ...
                                        if is_group2 == True: #group level 3
                                            ...
                                            for n_group3 in n_group2.node_tree.nodes: #nodes
                                                ...
                                                g_n3.append(g_out3); images.append(im3)
                                                ...
                                                if is_group3 == True: #group level 4
                                                    ...
                                                    for n_group4 in n_group3.node_tree.nodes: #nodes
                                                        ...
                                                        g_n4.append(g_out4); images.append(im4)
                    ...
                    #write data
                    #material attribs
                    t = datetime.now(); date_string = "{}/{}/{} at {}:{}:{} in {}".format(t.month, t.day, t.year, t.hour, t.minute, t.second, tzname[0])
                    ...
                    tree = ET.ElementTree(root); error2 = True
                    try:
...
def import_material(self, context):
    ...
    if temp_epath != "" and path.exists(bpy.path.abspath(temp_epath)) == True and temp_epath.endswith(".bmat"):
        ...
        if context.scene.import_materials_type == "2":
            temp_path = bpy.path.abspath(temp_epath).split("\\"); print(temp_path); del temp_path[len(temp_path) - 1]
            temp_end = temp_path[len(temp_path) - 1]; del temp_path[len(temp_path) - 1]
            folder_path = ""
            for i in temp_path: folder_path += i + "\\"
            ...
            for i in images:
                if i[0] not in bpy.data.images:
                    if root.attrib["Path_Type"] == "Relative":
                        path_list = context.scene.save_path_import.split("\\"); del path_list[len(path_list) - 1]
                        path_str = ""      
                ...
                for type in group:
                    if counter == 0: #nodes
                        ...
                        for node in type:
                            ...
                            temp.name = node.attrib["name"]; temp.label = node.attrib["label"]
                    ...
                    elif counter == 1: #links
                        for link in type:
                            ld = ast.literal_eval(link.attrib["link_info"])                    
                            if group.tag == "main":
                                o = nt[ld[0]].outputs[ld[1]]; i = nt[ld[2]].inputs[ld[3]]
                                mat.node_tree.links.new(o, i)                        
                            else:
                                o = nt.nodes[ld[0]].outputs[ld[1]]; i = nt.nodes[ld[2]].inputs[ld[3]]
                                nt.links.new(o, i)    
                    counter += 1            
