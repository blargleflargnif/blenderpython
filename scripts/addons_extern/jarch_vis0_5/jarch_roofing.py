...
def shingles_3tab(mode, wh, ow, slope):
    verts = []; faces = []
    #last set of vertices have to be put in separate mesh if this object is being cut by a boolean to avoid issues
    last_verts = []; last_faces = []
    ...
    while cur_y < end_y:        
        ...
        while cur_x < ow:
            ...
            if temp_offset == False and last_row_stay == True:                           
                v += [(cur_x, cur_y + sj, cur_z), (cur_x, cur_y + sj, cur_z + tsi)] 
                v += [(cur_x, cur_y + bj, cur_z), (cur_x, cur_y + bj, cur_z + tsi)]
                cur_x += ei
            ...
            for i in range(6): 
                ...
                if (cur_x <= ow and cut == False) or (cut == True and js == True):
                    js = False                         
                    v += [(cur_x, cur_y, cur_z + tsi), (cur_x, cur_y, cur_z + tei), (cur_x, cur_y + sj, cur_z), (cur_x, cur_y + sj, cur_z + tsi)]
            ...
            #update p to be used with last_verts not verts
            if last_shingle == True:
                p = len(last_verts)
    ...
    #adjust points for slope for last_verts
    last_verts2 = []
    
    for num in range(len(last_verts)):
        point = point_rotation((last_verts[num][1], last_verts[num][2]), (-(wh / 2), last_verts[num][2]), rot)
        last_verts2.append((last_verts[num][0], point[0], point[1]))
        
    return verts2, faces, last_verts2, last_faces
...
def create_roofing(self, context, mode, mat, shingles, tin, length, width, slope, res, radius):
    ...
    #shingles
        ...
        #3-tab shingles
            ...
            verts, faces, verts2, faces2 = shingles_3tab(mode, width, length, slope)         
    ...
    if mode == "add":
        ...
        #create second object if needed
        if verts2 != []:
            mesh2 = bpy.data.meshes.new("roofing2")
            mesh2.from_pydata(verts2, [], faces2)
            return_ob2 = bpy.data.objects.new("roofing2", mesh2)
            context.scene.objects.link(return_ob2)
        
    return return_ob, return_ob2 
...
def UpdateRoofing(self, context):            
    ...
    if use_ob.ro_object_add == "convert" and obs != []:
        ...
        for o_name in obs:
            ...
            new_ob, new_ob2 = create_roofing(self, context, roof_data[0], roof_data[1], roof_data[2], roof_data[3], xy_dims, z_dim, o.pl_pitch, roof_data[4], roof_data[5])
            ...
            #calculate offset between center of geomerty on new_ob and o.location so to know where to place new_ob2            
            new_ob2_loc = [o.location[0] - new_ob.location[0], o.location[1] - new_ob.location[1], o.location[2] - new_ob.location[2]]
            ...
            #adjust new_ob2's location, rotation, apply solidify, and boolean: all if necessary
            if new_ob2 != "":
                new_ob.select = False
                context.scene.objects.active = new_ob2
                new_ob2.select = True
                
                
                new_ob2.location = new_ob2_loc
                new_ob2.rotation_euler = eur
                
                #solidify new object
                if roof_data[1] == "1":
                    bpy.ops.object.modifier_add(type = "SOLIDIFY")
                    new_ob2.modifiers["Solidify"].thickness = 0.00028702
                    bpy.ops.object.modifier_apply(modifier = "Solidify", apply_as = "DATA")
                
                #boolean new object
                bpy.ops.object.modifier_add(type = "BOOLEAN")
                new_ob2.modifiers["Boolean"].object = o
                bpy.ops.object.modifier_apply(modifier = "Boolean", apply_as = "DATA")
                
                new_ob2.select = False
                new_ob.select = True
                context.scene.objects.active = new_ob                                       
            ...
            if o.name == use_ob.name:
                ...
                if new_ob2 != "":
                    roofing_names.append(new_ob2.name)            
    ...
    if m_ob.ro_object_add == "add":
        ...
        #create new object
        nothing, nothing2 = create_roofing(self, context, m_ob.ro_object_add, m_ob.ro_mat, m_ob.ro_shingles, m_ob.ro_tin, m_ob.ro_length, m_ob.ro_width, m_ob.ro_slope, m_ob.ro_res, m_ob.ro_tile_radius)
