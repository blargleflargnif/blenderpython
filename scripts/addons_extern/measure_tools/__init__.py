    ...
    def draw(self, context):
        ...
        if stat.com_use:
            ...
            if stat.com_method == 'VOLUME':
                if not stat.error_manifold and not stat.error_normals:
                    row = layout.row()
                    row.label(text=Vector3DToString(stat.com))
                    row = layout.row()
                    row.prop(stat, "com_draw", toggle=True)
                    row = layout.row()
                    row.operator("wm.com_to_empty")
...
# ------ Mesh Statistics ------ #	
                ...
                if stat.updating_locked:
                    row.operator("object.editmode_toggle", text="Enter Object Mode", icon='OBJECT_DATAMODE')
                    row.prop(stat, "updating_locked", icon_only=True, toggle=True, icon='LOCKED')
                else:
                    row.operator("object.editmode_toggle", text="Object Mode to Refresh", icon='FILE_REFRESH')
                    row.prop(stat, "updating_locked", icon_only=True, toggle=True, icon='UNLOCKED')
