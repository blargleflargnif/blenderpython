...
class RENDER_OT_RenderShots_create_folders(bpy.types.Operator):
    ...
    def execute(self, context):
        scn = context.scene
        init_props(scn)  # Is this a good place to defer the initialisation?
...
class RENDER_OT_RenderShots_settingshelp(bpy.types.Operator):
    ...
    cam = StringProperty()
    _cycles = None  # The Context is not set yet. Because of this - lazy initialisation
...
class RENDER_OT_RenderShots_constraints_add(bpy.types.Operator):
    ...
    def execute(self, context):
        init_props(context.scene)  # Is this a good place to defer the initialisation?
