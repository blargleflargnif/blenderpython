...
from va_view import check_view, check_view_context, \
                    make_snap_matrix, get_vector_from_snap_maxrix, \
                    convert_world_to_window, convert_window_to_world
from va_utils import get_matrix_element_square
from va_gl import draw_box, draw_triangles, draw_triangle_relative, \
...
# 初期化に使う値
default_config = d = OrderedDict()  # OrderedDict使う必要は特に無かった
d['scale_size'] = [5, 4, 4]  # 目盛りの大きさ(10, 偶数番目の目盛り, 奇数番目の目盛り)
d['number_min_px'] = [18, 40]  # 36 # 最小グリッドの大きさ(dot)がこの値より大きくなると最小グリッド*5(or最小グリッド)の位置に数値を表示します。
d['draw_mouse_coordinate'] = 1  # 0:off, 1:on マウスの座標を表示。要パッチ
d['autohide_mouse_coordinate'] = 1
d['autohide_MC_threthold'] = 2  # -1:off, 0以上:閾値として使用。マウスが座標表示BOXに近づくとBOXを隠します。
...
d['cursor_type'] = 'cross_scale'  # enum['none', 'cross', 'cross_scale']0 要パッチ
d['cursor_scale_size'] = [12, 6, 2]  # 目盛りの大きさ(main, sub1, sub2)
d['measure_cursor_scale_size'] = [16, 6, 6]  # 目盛りの大きさ(main, sub1, sub2)
d['measure_scale_size'] = [14, 5, 5]  # draw measure.rulers
d['measure_select_point_threthold'] = 30  # MeasureMode時、右クリックで選択可能な距離
d['draw_scale_type'] = 1  # 0:offz, 1:on 右上の文字
d['draw_unit'] = 1  # 0:offz, 1:on 右下の文字、最小グリッド分のBlenderUnit
d['color_main'] = [1.0, 1.0, 0.2, 1.0]  # [r, g, b, a]
d['color_number'] = d['color_main']
color_background = bpy.context.user_preferences.themes['Default'].view_3d.back
d['color_background'] = list(color_background) + [1.0]  # alpha
d['color_cursor'] = [1.0, 1.0, 1.0, 0.3]
d['color_cursor_bold'] = [1.0, 1.0, 1.0, 1.0]
#d['color_cursor_number'] = [1.0, 1.0, 1.0, 1.0]  # 未使用
d['color_measure'] = [0.9, 1.0, 0.7, 0.6]
d['color_measure_sub'] = [0.9, 1.0, 0.7, 0.3]  # 補助線 & ruler.draw_circle
...
class RulerConfig(bpy.types.PropertyGroup):
    ...
    autohide_MC_threthold = IntProperty(attr='autohide_MC_threthold',  # 'autohide_mouse_coordinate_threthold'は長すぎてエラー
                            name='Thretholed',
    ...
    def config_check(self):
        if not hasattr(bpy.context.window, 'event'): # patch適用後に利用可
...
class MeasurePoint():
    ...
    #dpu = 1.0 # dot per blender unit. 次のpointとの間の値
    #angle = 0.0 # 次のpointとの間の値。正負によって描画方向を変更(crossで判定)
    #cross = Vector([0, 0, 0])
    #length = 0.0
    offset = 0.0 # scale描画
    length_to_next = 0.0
...
class Measure():
    ...
    point_bak = None # Transform開始点
    rulers = []
    active_index = None
    mouse_point = MeasurePoint(Vector([0, 0, 0]))
    draw_circle = 1 # 1:基点が中心, 2:マウスが中心
    space_type = '3D' # or '2D'
    draw_shortcut = 0 # 左のショートカット一覧
    draw_column_max = 0 # 小数点以下の桁数。

    offset_angle = 20 # config。描画時pointからの距離
    offset_angle_2 = 50 # activeがrulerの中
    offset_length = 20
    text_height_max = 0 # width&height box 描画時、数値によって高さが変わるのを防ぐ
    draw_text_list = [] # [('text', (posx, posy, posz)), ...] # まとめて最後に描画

    # snap
    # ※region.width, region.heightが取る可能性の有る最大値を指定、升目を作成。
    # (Ctrlホールドの度にlistを作っていては時間が掛かるため)
    '''
    ...
    snap_imat = [[[] for j in range(snap_region_xsize)] \
                                               for i in range(snap_region_ysize)]
    '''
    snap_region_subdivide = 100 # 頂点をwindow座標によって100*100の升目(行列)に配置。
    snap_vmat = snap_imat = None

    snap_range = 20
    ctrl = 0 # hold
    snap_point = MeasurePoint(Vector([0, 0, 0])) # world座標.Shift押しによって2DSnap,3DSnap切り替え
    snap_find = 0 # 0:None, 2:snap2D, 3:snap3D
    snap_to_visible = 0 # 0:snap to active & selected, 1:snap to ob.is_visible
    snap_to_dm = 1 # active&EditModeでのみ機能. ob.create_mesh(context.scene, 1, 'PREVIEW')
    #不要snap_me = None # ob.create_mesh(context.scene, 1, 'PREVIEW')
    ...
    def update_snap_me(self, ob):
        ...
        if self.snap_to_dm:
            self.snap_me = ob.create_mesh(bpy.context.scene, 1, 'PREVIEW') # local座標
        else:
...
# pass_keyboard_inputs: measureモード時にblenderに渡すショートカット(他は無効化)
...
pass_keyboard_inputs.extend([ShortCut('A', oskey=True), ShortCut('S', oskey=True),
                             ...
                             ShortCut('Z'), ShortCut('Z', shift=True),
                             ShortCut('Z', alt=True),
...
class Data():
    ...
    unit_pow = 0 # 累乗 unit = 10 ** unit_pow = dx分の増加量
    offset = [0, 0, 0] # originのwindow座標
    offset_3d = Vector() # originの3d座標 center_ofsかlocal_grid_originを使用
    view = 'top'
    center_ofs = Vector([0,0,0]) # ルーラー零位置の3D座標

    # Scale描画
    draw_offset_x = draw_offset_y = 0.0 # window端から外側の目盛りまでの距離
    start_number_x = start_number_y = 0.0 # 最初の目盛りの数値
    unit_x = unit_y = 1.0 # unitに符号を付けた物
    ...
    def update(self, class_self, context, measure_mode=False):
        if measure_mode:
            # measure_mode時に必要な分だけ更新
            window = context.window
            event = window.event
            region = get_active_region_id(event, return_data=True)
            self.in_region = False # 同Region内のみマウスクリック反応。(同Are内は再描画)
            if not region:
                return
            rv3d = region.region_data # patch
            if not rv3d:
                #マウス座標だけ更新
                self.mouseco = mouseco = Vector([event.mouse_x - self.region.xmin, \
                                             event.mouse_y - self.region.ymin, 0])
                return
            self.region = region
            self.sx = region.width
            self.sy = region.height
            self.persmat = rv3d.perspective_matrix
            self.mouseco = mouseco = Vector([event.mouse_x - region.xmin, \
                                             event.mouse_y - region.ymin, 0])
            self.in_region = 0 <= mouseco[0] <= self.sx - 1  and \
                             0 <= mouseco[1] <= self.sy - 1
        ...
        window = context.window
        if hasattr(context.window, 'event'):
            event = window.event
        else:
            event = None
        ...
        if event is not None:
            mouseco = Vector([event.mouse_x - region.xmin, event.mouse_y - region.ymin, 0])
        ...
        self.dot_per_blender_unit, self.dx, self.unit_pow = \
                              get_dx(sx, sy, persmat, self.viewinvmat, self.viewloc)
        ...
        self.set_scale_draw_options() # scale描画に必要な値を計算 (関数でも良かった？
        return True
    ...
    def set_scale_draw_options(self):
        ...
        vec[1] += (self.sy / 2) / dot_per_blender_unit # 画面左下基準
...
def get_localgrid_status(spaceview3d):
    ...
    if hasattr(spaceview3d, 'use_local_grid'):
        localgrid = spaceview3d.use_local_grid
    else:
        localgrid = False
    if localgrid:
        localgrid_orig = spaceview3d.local_grid_origin
        localgrid_quat = spaceview3d.local_grid_rotation
    else:
        localgrid_orig = localgrid_quat = None
    return localgrid, localgrid_orig, localgrid_quat

def get_dx(sx, sy, persmat, viewinvmat, viewloc):
...
def draw_scale(data, config):
    ...
    unit_x = data.unit_x # 符号付き
    unit_y = data.unit_y # 符号付き
...
def draw_measure_status(data, config, measure):
    ...
    if measure.draw_shortcut:
        text_list +=['Toggle: Ctrl + Shift + R',
                     ...
                     'Delete All: Shift + X',
                     '',
                     'Snap to Mesh 2D: Ctrl',
                     'Snap to Mesh 3D: Ctrl + Alt',
                     '',
                     'TotalMode: T',
                     'Draw Circle: C',
                     'Draw Length: L',
...
def draw_measure_box(data, config, font, measure):
    ...
    if active_to_mouse_vec[1] >= 0.0:
        y1 = active_vec_window[1] + text_height_max + font.offset * 2 # 下端
        height_box_y = y1 + text_height_max + font.offset * 2 # right line 描画に使用
    else:
        y1 = active_vec_window[1] - text_height_max * 2 - font.offset * 4 # 下端
        height_box_y = y1
    ...
    for ruler in measure.rulers + rulers_ex:
        ...
        for p1, p2 in zip(ruler, ruler[1:]):
            if measure.space_type == '3D':
                ...
                # window座標におけるZ値を同一にしたVector(world)を作る
    ...
    for ruler in measure.rulers + rulers_ex:
        ...
        for p1, p2 in zip(ruler, ruler[1:]):
            ...
            if len(cross_vs) >= 2:
                ...
                start_point = MeasurePoint(vec1 + vec12 * f) # 画面端と交差するなら、その交点
                f = d2 / d12 if d12 else 0.0
                end_point = MeasurePoint(vec1 + vec12 * f) # 画面端と交差するなら、その交点
                if d1 > d2:
                    d1, d2 = d2, d1
                ...
                d1 = (cross_vs[0] - v1).length
                f = d1 / d12 if d12 else 0.0
                start_point = MeasurePoint(vec1 + vec12 * f) # p1が外側の場合を想定
                end_point = MeasurePoint(vec2)
                if 0 <= v1[0] < sx and 0 <= v1[1] < sy: # p1が内側だった場合
                    #d1 = d12 - d1
                    d1 = 0.0
                ...
                total_length += l12
                continue
            #if d12 == 0.0 or l12 == 0.0: # 3次元ベクトルをベクトル方向から見る場合に描画が必要
            #    continue

            # set offset
            dpu = d12 / l12 if l12 else 0.0 # line毎に更新 dot per blender unit
            ...
            v12nv = Vector([v12n[1], -v12n[0]]) # v12nと垂直
            unit = 10 ** data.unit_pow # 1目盛り分のBlenderUnit

            # start
            v = start_point.vec_window.to_2d() # scale描画開始位置
            if start_point.offset > 1E-8: # total mode で左側に余分なscaleが描画される為
            ...
            for cnt in range(cnt_start, cnt_last + 1):
                ...
                elif cnt % 10 != 0 and cnt% 5 == 0: # 1/2の目盛り
                    l = 3
    ...
    if measure.on and acti and measure.drag == 0:
        ...
        if len(active_ruler) >= 2:
            if active_point == active_ruler[0]:
                ...
                r1 = MeasureRuler([point_b, active_point, mouse_point], f=1) # 特殊
    ...
    if acti:
        if active_ruler.total_mode:
            if active_point == active_ruler[-1] or \
               active_point == active_ruler[0]: # マウス付近にtotal_length表示
                mouse_point.f = 1
        if measure.drag == 0:
            if measure.on:
                if active_ruler.total_mode:
        ...
        elif measure.drag == 2: # measure.on==0の時は必ずdrag=0
            point_bak = measure.point_bak
...
def draw_measure(data, config, measure):
    ...
    # update & reset
    measure.update_points_vec_window(data) # window座標のみ更新
...
def get_active_region_id(event, return_data=False):
    # modal中にcontext.regionを呼び出すと不具合
    mouseco = [event.mouse_x, event.mouse_y]
    for area in bpy.context.screen.areas:
        for region in area.regions:
            if region.xmin <= mouseco[0] <= region.xmax and \
               region.ymin <= mouseco[1] <= region.ymax:
                if return_data:
                    return region
                else:
                    return region.id
...
def draw_callback_px(self, context):
    ...
    if config.draw_scale_type:
        draw_scale_type(data, config) # 右上の文字

    if config.draw_unit:
        draw_unit(data, config) # 右下の文字
...
class VIEW3D_OT_display_ruler(bpy.types.Operator):
    ...
    def add_handler(self, context):
        if context.area.type == 'VIEW_3D':
            #if not self.bl_label in context.window_manager.operators.keys(): # invokeでFINISHEDを返さないと追加されない
            ...
            for region in context.area.regions:
                if region.type == 'WINDOW':
                    break
            if not self.__class__.added:
                '''起動中に実行するのは必ず一度だけ。追加した分だけdraw_callback_pxが呼ばれる
                対象のregionがbpy.data.screens中に無くなっても呼び出される
                参考 self.__class__.region.callback_remove(handle)
                '''
                self._handle = region.callback_add(draw_callback_px,
                                               (self, context),
    ...
    def modal(self, context, event):
                ...
                if hasattr(context.window, 'event'): # patch
                    ...
                    if screen.name in self.__class__.disable_regions:
                        if region.id in self.__class__.disable_regions[screen.name]:
                            toggle = False
                    if toggle and region.region_data:
                        # drag cancel
                        if measure.drag:
                            if measure.drag == 2:
                                point = measure.active('point')
        ...
        if measure.on:
            ...
            #mouseco = data.mouseco # window co (3D) z値は常に0

            measure.update_mouse_point(data) # measure.update_points_vec_window()はこの際必要無い
            mouseco = measure.mouse_point.vec_window
            ...
            if event.ctrl:
                ...
                # mesh_fake_knife.pyの物を改良
                region = data.region
                sx = region.width
                sy = region.height
                persmat = region.region_data.perspective_matrix
                ...
                if measure.ctrl == 0:
                    ...
                    measure.snap_vmat = make_snap_matrix(sx, sy, persmat, \
                ...
                # 横方向の要素:行row, 縦方向の要素:列col
                snap_range = measure.snap_range
                snap_vec = get_vector_from_snap_maxrix(mouseco, max(sx, sy), \
                                                       subdivide, snap_vmat, \
                                                       snap_range)
                if snap_vec:
                    if event.alt == 0: # snap 2D
            ...
            if event.type == 'MOUSEMOVE':
                ...
                if event.value == 'PRESS':
                    if measure.drag == 0:
                        if measure.rulers:
                            # change Active Point
                            measure.update_points_vec_window(data) # 座標計算
                ...
                if measure.drag == 0:
                    #vec_world = measure.update_mouse_point(data) # 切り替え後mouseを動かさずに押す場合に必要
                    if event.ctrl and measure.snap_find:
            ...
            elif event.type == 'S' and get_modifier(event) == 0:
                if event.value == 'PRESS':
                ...
                if event.value == 'PRESS':
                    ...
                    if ruler:
                        if len(ruler) >= 2 and modifier == 0:
                            ...
                            active_index = measure.active_index
                            if active_index[1] >= len(ruler): # 範囲外
            ...
            if not data.in_region: # 3DView内にマウスがあるか？
                # 3DView以外なら入力をそのまま渡す
                return {'PASS_THROUGH'}
            else:
                return {'RUNNING_MODAL'}
...
class VIEW3D_PT_ruler(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator('view3d.display_ruler', text='Enable').mode=1
        row.operator('view3d.display_ruler', text='Disable').mode=-1
