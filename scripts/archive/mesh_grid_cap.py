...
def grid_calculation():
    good_selection = False #used to say if the vertices selection is appropriate for the script execution  
    ...
    if len(corners) == 4:   #se i corners sono esattamente 4 #if corners are exactly 4
        ...
        if len(lato1) > 0 and len(lato2) > 0 and len(latoA) > 0 and len(latoB) > 0:                                        
            ...
            def trova_punti(vert1, vert2, vertA, vertB):
                ...
                return(mathutils.geometry.intersect_line_line(p1, p2, pA, pB))
...
class MESH_OT_grid_cap(bpy.types.Operator):
    ...
    def __init__(self):        
        self.good_selection = grid_calculation() #calculate the upper and lower grids just one time        
