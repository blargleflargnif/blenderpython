bl_info = {
    "name": "create different sort of faces",
    "author": " PKHG ",
    "version": (0, 0, 5),
    "blender": (2, 7, 1),
...
class VIEW3D_Faces_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        layout = self.layout
        layout.operator(AddFaces.bl_idname, "Add Faces!")
        layout.label("sel.: active mesh")
        layout.label("works on sel. faces")
        layout.label("start in OBJECT mode")
...
class pkhg_help(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        layout.label('Choose from the options to create face types.')
        layout.label('Warning')
        layout.label('Experimental')
    def execute(self, context):
        return {'FINISHED'}
